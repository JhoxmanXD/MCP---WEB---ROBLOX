...
En desarrollo el estado usa `AGENT_STATE_BACKEND=memory`. En producción puede usar `AGENT_STATE_BACKEND=redis` con `AGENT_STATE_URL` secreto y `AGENT_STATE_NAMESPACE`; el snapshot JSON está versionado como `agent-state-v1`, tiene TTL y se protege con lock distribuido más CAS. Las mutaciones y el Execute de Prepared se serializan cross-worker, y los jobs/catálogo mínimos que alimentan el resultado también viajan en el snapshot. Si el backend configurado cae, `/agent/*` devuelve `503 AGENT STATE BACKEND UNAVAILABLE` y no hace fallback silencioso a memoria. Comprueba `agent_state_backend` en `/agent/status` o `/api/v1/health.json`.

La revisión del Draft es exclusivamente la revisión semántica de sus argumentos. Renderizar Views, abrir Composers/Editores/Picker, registrar Actions, tocar TTL, persistir Redis o crear Prepared no la incrementa. Una Action solo avanza la revisión cuando cambia el hash canónico de argumentos; `DRAFT_REVISION_ADVANCED` y `DRAFT_REVISION_UNCHANGED` dejan la evidencia de cada intento.
