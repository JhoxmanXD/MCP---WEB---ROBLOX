...
La revisión del Draft es semántica y exclusiva de los argumentos. Render, creación de snapshots, registro de Actions, apertura de Composers/Editores/Picker, TTL, persistencia Redis y Prepared no la avanzan; una Action solo suma uno si cambia el hash canónico. Los eventos `DRAFT_REVISION_ADVANCED` y `DRAFT_REVISION_UNCHANGED` permiten reconstruir cualquier transición y separar esta revisión del CAS de almacenamiento.
